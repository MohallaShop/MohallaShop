...
export function CategoryCard({ category, href }: { category: CategorySummaryLike; href: string }) {
  return (
    <Link
      href={href}
      className="border-border bg-surface shadow-card hover:shadow-elevated group flex min-w-0 items-center gap-3 rounded-2xl border p-3 transition sm:gap-4 sm:p-4 md:hover:-translate-y-0.5"
    >
      <span className="bg-brand-500/10 text-brand-600 dark:text-brand-400 border-border/50 grid h-10 w-10 shrink-0 place-items-center rounded-lg border sm:h-12 sm:w-12 sm:rounded-xl">
        <GridIcon className="h-5 w-5 sm:h-6 sm:w-6" />
      </span>
      <span className="min-w-0">
        <span className="text-content block truncate text-sm font-semibold sm:text-base">
          {category.name}
        </span>
